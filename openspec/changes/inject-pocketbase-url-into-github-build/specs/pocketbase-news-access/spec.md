...
#### Scenario: CI build injects required URL
- **GIVEN** the GitHub Actions deployment workflow builds the frontend
- **WHEN** the build step executes `npm run build`
- **THEN** `VITE_POCKETBASE_URL` is injected from repository/environment configuration and the build does not rely on any in-repo fallback value
